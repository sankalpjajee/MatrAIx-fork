...
def _read_trial_instruction_markdown(trial_dir: Path, repo_root: Path) -> str | None:
    ...
    task_instruction = _read_trial_task_doc(trial_dir, "task_instruction.md", repo_root)
    if task_instruction:
        return task_instruction
    for candidate in (trial_dir / "instruction.md",):
...
def _render_persona_prompt(
    ...
    if raw:
        ...
        if block and not _is_thin_persona_prompt(block, persona=persona):
            return "## Persona\n{}".format(block).strip()
    from persona_eval.user_sim.prompt import render_persona_block
...
def _map_chatbot_debrief(
    *,
    ...
    result = build_result_from_harbor_artifacts(
        ...
        config=PersonaEvalConfig(domain=domain, engine="gpt-4o-mini"),
        persona=persona,
