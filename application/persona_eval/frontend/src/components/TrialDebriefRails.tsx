      ...
      {open && hasBody ? (
        <div className="border-t border-outline/40 bg-surface/20 px-4 py-3">
          <div className="custom-scrollbar max-h-96 overflow-auto">
            <Markdown className="max-w-none text-[12px] leading-relaxed text-text-variant">{body}</Markdown>
          </div>
        </div>
      ) : null}
    </section>
...
export function TrialDebriefRails({
  ...
  const questionnaireBody = normalizeQuestionnaireMarkdown(questionnaireMarkdown);
  const outputSchemaBody = normalizeOutputSchemaMarkdown(outputSchemaMarkdown);
  ...
  if (personaBody || persona?.name || persona?.id || persona?.source) {
    rails.push(
      <DebriefRail
        ...
        icon="person"
        subtitle={persona?.name ?? persona?.id ?? null}
        badge={persona?.source ?? null}
        body={personaBody}
      />,
    );
  }
  ...
  if (questionnaireBody) {
    rails.push(<DebriefRail key="questionnaire" label="Questionnaire" icon="list_alt" body={questionnaireBody} />);
  }
  if (outputSchemaBody) {
