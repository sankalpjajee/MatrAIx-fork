...
CELLS = REPO / "application/validation/results/cells"
...
def main():
    ...
    lines.append("400 trials = 10 attributes × 4 envs × (5 positive + 5 negative personas). "
                 "Each trial's agent trajectory/artifact is LLM-judged (Opus 4.8 via CAPI) for "
    ...
    for a in ATTRS:
        ...
        for e in ENVS:
            ...
            report.setdefault(a, {})[e] = {"pos_expressed": p, "pos_n": np_,
                                           "neg_expressed": n, "neg_n": nn}
    ...
    for a in ATTRS:
        ...
        lines.append(f"- **{a}**")
        if pe: lines.append(f"  - pos ({pe.get('value')}): {pe['evidence'][:160]}")
        if ne: lines.append(f"  - neg ({ne.get('value')}): {ne['evidence'][:160]}")
    lines.append("")
    lines.append("## Notes")
    lines.append("- **survey** is the cleanest env (persona generates text directly), so attribute "
                 "adherence is most faithful there.")
    lines.append("- **web** (openhands) is weakest — the agent often stops early / under-produces, "
                 "an execution-layer limit, not a persona-adherence failure.")
    lines.append("- **cog_storytelling** negative ('None') is hard: agents default to using examples, "
                 "so suppressing storytelling is rarely achieved — a genuine attribute-difficulty finding.")
    lines.append("- Coding attributes (comment/naming/summary) validate cleanly in survey/chat, "
