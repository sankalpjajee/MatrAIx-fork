...
ENVS = {
    ...
    "osapp-linux": {
        ...
        "model": "openai/claude-opus-4.8",
        "extra": "api_base: https://api.githubcopilot.com",
    },
}
...
def run_trial(attr_slug, env, pol, pid, val, tok, idx=0):
    ...
    env_vars["CAPI_API_KEY"] = tok
    env_vars["CAPI_BASE_URL"] = "https://api.githubcopilot.com"
    ...
    if env in ("web", "osapp-linux"):
        ...
        env_vars["OPENAI_BASE_URL"] = "https://api.githubcopilot.com"
        env_vars["LLM_API_KEY"] = tok
        env_vars["LLM_BASE_URL"] = "https://api.githubcopilot.com"
    if env == "chat":
        env_vars.pop("ANTHROPIC_API_KEY", None)
        env_vars["ANTHROPIC_AUTH_TOKEN"] = tok
        env_vars["ANTHROPIC_BASE_URL"] = "https://api.githubcopilot.com"
    ...
    try:
        subprocess.run(
            [
                ...
                "run",
                "-c",
                f"application/validation/recipes/_matrix/{job}.yaml",
            ],
            cwd=REPO,
            env=env_vars,
            timeout=to,
