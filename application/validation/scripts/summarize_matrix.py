...
OUT = REPO / "application/validation/results/matrix_summary.json"
...
def main():
    ...
    def verdict(cell):
        ...
        if p_ok and n_ok: return "✅"
        if p_ok or n_ok: return "◐"
        return "✗"
    ...
    for a in ATTRS:
        ...
        for e in ENVS:
            ...
            if cell: summary.setdefault(a, {})[e] = {
                "pos": [{"expressed":t.get("expressed"),"conf":t.get("confidence")} for t in cell["pos"]],
                "neg": [{"expressed":t.get("expressed"),"conf":t.get("confidence")} for t in cell["neg"]],
    ...
    print("\ncells:", dict(Counter(allv)), "  (✅ pos&neg both expressed, ◐ one side, ✗ neither)")
