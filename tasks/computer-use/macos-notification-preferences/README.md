...
uv run harbor run \
  ...
  -p tasks/computer-use/macos-notification-preferences \
  -e use-computer
```
...
```bash
uv run harbor run -c configs/jobs/persona-computer-use-macos-local.yaml
```

Oracle check (no LLM; writes the decision file directly):

```bash
uv run harbor run -p tasks/computer-use/macos-notification-preferences -a oracle -e use-computer
```
